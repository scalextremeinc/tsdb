...
public final class Tags {
  ...
  static String getValue(final TSDB tsdb, final byte[] row,
    ...
    final byte[] id = tsdb.getTagNames().getId(name);
    ...
    try {
      return tsdb.getTagValues().getName(value_id);
  ...
  static byte[] getValueId(final TSDB tsdb, final byte[] row,
    ...
    for (short pos = (short) (tsdb.getMetrics().width() + Const.TIMESTAMP_BYTES);
         pos < row.length;
  ...
  static Map<String, String> getTags(final TSDB tsdb,
    ...
    final short metric_ts_bytes = (short) (tsdb.getMetrics().width()
    ...
    for (short pos = metric_ts_bytes; pos < row.length; pos += tag_bytes) {
      ...
      final String name = tsdb.getTagNames().getName(tmp_name);
      System.arraycopy(row, pos + name_width, tmp_value, 0, value_width);
      final String value = tsdb.getTagValues().getName(tmp_value);
  ...
  private
    ...
    for (final Map.Entry<String, String> entry : tags.entrySet()) {
      ...
      final byte[] value_id = (create
                               ? tsdb.getTagValues().getOrCreateId(entry.getValue())
                               : tsdb.getTagValues().getId(entry.getValue()));
