...
public final class TsdbHbase implements TSDB {
  ...
  public KeyValue compact(final ArrayList<KeyValue> row) {
   ...
   */
  public void scheduleForCompaction(final byte[] row, final int base_time) {
  ...
  public Deferred<ArrayList<KeyValue>> get(final byte[] key) {
    return client.get(new GetRequest(table, key));
  }

  /** Puts the given value into the data table. */
  public Deferred<Object> put(final byte[] key,
  ...
  public Deferred<Object> delete(final byte[] key, final byte[][] qualifiers) {
